...
class EdgePose {
public:
  EdgePose(unsigned int i, unsigned int j, const Eigen::Isometry3d &delta_pose,
           const Matrix6d &info)
...
class EdgePoseError {
public:
  EdgePoseError(const Eigen::Isometry3d &delta_ij,
  ...
  static ceres::CostFunction *Create(const Eigen::Isometry3d &pose_ij,
                                     const Matrix6d &sqrt_information) {
...
class EdgeGNSSError {
public:
  EdgeGNSSError(const Eigen::Vector3d &position,
  ...
  template <typename T>
  bool operator()(const T *const position, T *residual_ptr) const {
  ...
  static ceres::CostFunction *Create(const Eigen::Vector3d &position,
                                     const Eigen::Matrix3d &sqrt_information) {
    return new ceres::AutoDiffCostFunction<EdgeGNSSError, 3, 3>(
        new EdgeGNSSError(position, sqrt_information));
  }
...
public:
  ...
  static ceres::CostFunction *Create(const double &delta_height,
...
public:
  ...
  static ceres::CostFunction *Create(const double &height,
                                     const double &sqrt_information) {
    return new ceres::AutoDiffCostFunction<EdgeAbsHeightError, 1, 3>(
        new EdgeAbsHeightError(height, sqrt_information));
  }
...
public:
  EdgeRollPitchError(const Eigen::Vector2d &rp,
                     const Eigen::Matrix2d &sqrt_information)
      : roll_pitch_(rp), sqrt_information_(sqrt_information) {}

  template <typename T>
  bool operator()(const T *const orientation_i, const T *const orientation_j,
                  T *residual_ptr) const {
    Eigen::Map<const Eigen::Quaternion<T>> q_i(orientation_i);
    Eigen::Map<const Eigen::Quaternion<T>> q_j(orientation_j);
  ...
  static ceres::CostFunction *Create(const Eigen::Vector2d &rp,
                                     const Eigen::Matrix2d &sqrt_information) {
    return new ceres::AutoDiffCostFunction<EdgeRollPitchError, 2, 4, 4>(
        new EdgeRollPitchError(rp, sqrt_information));
  }
...
private:
  ...
  template <typename T>
  Eigen::Matrix<T, 3, 1> toEulerAngle(const Eigen::Quaternion<T> &q) const {
...
public:
  ...
  EdgePoseSE3CostFunction(Sophus::SE3 meas, Matrix6d sqrt_info)
      : measurment_(meas), sqrt_information_(sqrt_info) {}
  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals,
    ...
    if (jacobians) {
      if (jacobians[0]) {
        Eigen::Map<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>> jacobian_i(
            jacobians[0]);
      ...
      if (jacobians[1]) {
        Eigen::Map<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>> jacobian_j(
            jacobians[1]);
        Matrix6d J = JRInv(Sophus::SE3::exp(residual));
