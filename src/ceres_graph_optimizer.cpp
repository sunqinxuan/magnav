...
CeresGraphOptimizer::CeresGraphOptimizer(const YAML::Node &node) {
  ...
  minimizer_progress_to_stdout_ =
      node["minimizer_progress_to_stdout"].as<bool>();
  kernel_type_ = node["kernel_type"].as<std::string>();
  kernel_size_ = node["kernel_size"].as<double>();
  ceres_debug_ = node["ceres_debug"].as<bool>();
  if (node["use_preint_restrain"].as<bool>())
    weight_preint_edge_ = node["weight_preint_edge"].as<double>();
  if (node["use_loop_close"].as<bool>())
    weight_loop_edge_ = node["weight_loop_edge"].as<double>();
  if (node["use_gnss"].as<bool>())
    weight_gnss_edge_ = node["weight_gnss_edge"].as<double>();
}
...
bool CeresGraphOptimizer::EstimateAndCullingWrongLoop(ceres::Problem *problem) {
  ...
  if (Residuals.size() != loop_res_ids_.size() * 6)
    return false;
  ...
  WARNING("[CeresGraphOptimizer][EstimateAndCullingWrongLoop] Find ",
          Loss.size() - bad_case_num, " inliers!");
  WARNING("[CeresGraphOptimizer][EstimateAndCullingWrongLoop] Find ",
          bad_case_num, " outliers!");
...
bool CeresGraphOptimizer::BuildCeresProblem(ceres::Problem *problem) {
  ...
  for (DequeEdgePose::const_iterator it = edges_.begin(); it != edges_.end();
       ++it) {
    ...
    if (it_vertex_i == vertices_.end()) {
      edges_.erase(it);
      // WARNING("[ceres][odom_constraint] vertex with id ", edge.id_i, "not
      // found!");
      continue;
    }
    ...
    if (it_vertex_j == vertices_.end()) {
      edges_.erase(it);
      // WARNING("[ceres][odom_constraint] vertex with id ", edge.id_j, "not
      // found!");
      continue;
    }
    ...
    ceres::CostFunction *cost_function =
        EdgePoseError::Create(edge.pose_ij, sqrt_info);

    problem->AddResidualBlock(cost_function, loss_function_,
                              it_vertex_i->second.position.data(),
                              it_vertex_i->second.quaternion.coeffs().data(),
                              it_vertex_j->second.position.data(),
                              it_vertex_j->second.quaternion.coeffs().data());

    problem->SetParameterization(it_vertex_i->second.quaternion.coeffs().data(),
                                 quat_param);
    problem->SetParameterization(it_vertex_j->second.quaternion.coeffs().data(),
                                 quat_param);
  ...
  for (DequeEdgePose::const_iterator it = edges_preint_.begin();
       it != edges_preint_.end(); ++it) {
    const EdgePose &edge = *it;
    MapVertexPose::iterator it_vertex_i = vertices_.find(edge.id_i);
    if (it_vertex_i == vertices_.end()) {
      edges_preint_.erase(it);
      // WARNING("[backend][odom_constraint] vertex with id ", edge.id_i, "not
      // found!");
      continue;
    }
    ...
    if (it_vertex_j == vertices_.end()) {
      edges_preint_.erase(it);
      // WARNING("[backend][odom_constraint] vertex with id ", edge.id_j, "not
      // found!");
      continue;
    }
...
bool CeresGraphOptimizer::BuildCeresProblemSE3(ceres::Problem *problem) {
  ...
  for (DequeEdgePose::const_iterator it = edges_.begin(); it != edges_.end();
    ...
    ceres::CostFunction *cost_function =
        new EdgePoseSE3CostFunction(edge_se3, sqrt_info);

    problem->AddResidualBlock(cost_function, loss_function_,
                              it_vertex_i->second.xi.data(),
...
bool CeresGraphOptimizer::AddConstraints(ceres::Problem *problem) {
  ...
  if (edges_gnss_.size() == 0 && edges_roll_pitch_.size() == 0 &&
      edges_rel_height_.size() == 0 && edges_abs_height_.size() == 0 &&
      edges_loop_.size() == 0) {
    return false;
  }
  ...
  for (DequeEdgePose::const_iterator it = edges_loop_.begin();
       it != edges_loop_.end(); ++it) {
    const EdgePose &edge = *it;
    MapVertexPose::iterator it_vertex_i = vertices_.find(edge.id_i);
    if (it_vertex_i == vertices_.end()) {
      // WARNING("[backend][odom_constraint] vertex with id ", edge.id_i, "not
    ...
    auto loop_id = problem->AddResidualBlock(
        cost_function, loss_function_, it_vertex_i->second.position.data(),
        it_vertex_i->second.quaternion.coeffs().data(),
        it_vertex_j->second.position.data(),
        it_vertex_j->second.quaternion.coeffs().data());
  ...
  for (DequeEdgePose::const_iterator it = edges_gnss_.begin();
       it != edges_gnss_.end(); ++it) {
    const EdgePose &edge = *it;
    MapVertexPose::iterator it_vertex = vertices_.find(edge.id_i);
    if (it_vertex == vertices_.end()) {
      edges_gnss_.erase(it);
      // WARNING("[ceres][gnss_constraint] vertex with id ", edge.id_i, "not
      // found!");
      continue;
    }
    ...
    Eigen::Matrix3d sqrt_info =
        edge.information.block<3, 3>(0, 0).llt().matrixL();
    sqrt_info *= weight_gnss_edge_;
    ceres::CostFunction *cost_function =
        EdgeGNSSError::Create(edge.pose_ij.translation(), sqrt_info);

    problem->AddResidualBlock(cost_function, loss_function_,
                              it_vertex->second.position.data());
  }

  for (DequeEdgePose::const_iterator it = edges_rel_height_.begin();
       it != edges_rel_height_.end(); ++it) {
    const EdgePose &edge = *it;
    MapVertexPose::iterator it_vertex_i = vertices_.find(edge.id_i);
    if (it_vertex_i == vertices_.end()) {
      edges_rel_height_.erase(it);
      // WARNING("[ceres][rel_height_constraint] vertex with id ", edge.id_i,
      // "not found!");
      continue;
    }
    MapVertexPose::iterator it_vertex_j = vertices_.find(edge.id_j);
    if (it_vertex_j == vertices_.end()) {
      edges_rel_height_.erase(it);
      // WARNING("[ceres][rel_height_constraint] vertex with id ", edge.id_j,
      // "not found!");
      continue;
    }
    ...
    ceres::CostFunction *cost_function =
        EdgeRelHeightError::Create(edge.pose_ij.translation()(2), sqrt_info);
  ...
  for (DequeEdgePose::const_iterator it = edges_abs_height_.begin();
       it != edges_abs_height_.end(); ++it) {
    const EdgePose &edge = *it;
    MapVertexPose::iterator it_vertex = vertices_.find(edge.id_i);
    if (it_vertex == vertices_.end()) {
      edges_abs_height_.erase(it);
      // WARNING("[ceres][abs_height_constraint] vertex with id ", edge.id_i,
      // "not found!");
      continue;
    }
    ...
    const double sqrt_info = sqrt(edge.information(2));
    ceres::CostFunction *cost_function =
        EdgeAbsHeightError::Create(edge.pose_ij.translation()(2), sqrt_info);

    problem->AddResidualBlock(cost_function, loss_function_,
                              it_vertex->second.position.data());
  }

  for (DequeEdgePose::const_iterator it = edges_roll_pitch_.begin();
       it != edges_roll_pitch_.end(); ++it) {
    const EdgePose &edge = *it;
    MapVertexPose::iterator it_vertex_i = vertices_.find(edge.id_i);
    if (it_vertex_i == vertices_.end()) {
      edges_roll_pitch_.erase(it);
      // WARNING("[ceres][roll_pitch_constraint] vertex with id ", edge.id_i,
      // "not found!");
      continue;
    }
    MapVertexPose::iterator it_vertex_j = vertices_.find(edge.id_j);
    if (it_vertex_j == vertices_.end()) {
      edges_roll_pitch_.erase(it);
      // WARNING("[ceres][roll_pitch_constraint] vertex with id ", edge.id_j,
      // "not found!");
      continue;
    }
    ...
    const Eigen::Matrix2d sqrt_info =
        edge.information.block<2, 2>(0, 0).llt().matrixL();
    ceres::CostFunction *cost_function = EdgeRollPitchError::Create(
        edge.pose_ij.translation().block<2, 1>(0, 0), sqrt_info);
...
bool CeresGraphOptimizer::EstimateCovariance(ceres::Problem *problem) {
  ...
  for (MapVertexPose::const_iterator it = vertices_.begin();
       it != vertices_.end(); ++it) {
  ...
  for (MapVertexPose::iterator it = vertices_.begin(); it != vertices_.end();
       ++it) {
    const double *p = it->second.position.data();
    const double *q = it->second.quaternion.coeffs().data();
    ...
    it->second.covariance.block<3, 3>(0, 0) =
        Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(cov_pp);
    it->second.covariance.block<3, 3>(2, 2) =
        Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(cov_qq);
    it->second.covariance.block<3, 3>(0, 2) =
        Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(cov_pq);
    it->second.covariance.block<3, 3>(2, 0) =
        it->second.covariance.block<3, 3>(0, 2).transpose();
  }
...
bool CeresGraphOptimizer::EstimateCovarianceSE3(ceres::Problem *problem) {
  ...
  for (MapVertexPose::iterator it = vertices_.begin(); it != vertices_.end();
    ...
    it->second.covariance =
        Eigen::Map<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>>(cov_pp);
  }

  return true;
}

bool CeresGraphOptimizer::GetOptimizedPose(
    std::deque<Eigen::Isometry3f> &optimized_pose) {
...
CeresGraphOptimizer::GetOptimizedCovariance(int vertex_index) {
  Eigen::Matrix<float, 6, 6> cov;
  for (int i = 0; i < 6; i++)
    for (int j = 0; j < 6; j++)
      cov(i, j) = float(vertices_[vertex_index].covariance(i, j));

  return cov;
}

bool CeresGraphOptimizer::UpdateGraphNode() { return true; }

int CeresGraphOptimizer::GetNodeNum() { return vertices_.size(); }

void CeresGraphOptimizer::AddSe3Node(const Eigen::Isometry3f &pose,
                                     bool need_fix) {
  unsigned int vertex_index = vertices_.size();
  AddSe3Node(pose, vertex_index, need_fix);
}
void CeresGraphOptimizer::AddSe3Node(const Eigen::Isometry3f &pose,
                                     unsigned int vertex_index, bool need_fix) {
  vertices_.insert(std::pair<unsigned int, VertexPose>(
      vertex_index, VertexPose(vertex_index, pose.cast<double>(), need_fix)));
}
...
void CeresGraphOptimizer::AddSe3Edge(int vertex_index1, int vertex_index2,
  ...
  edges_.push_back(EdgePose(vertex_index1, vertex_index2,
                            relative_pose.cast<double>(), info));
}
void CeresGraphOptimizer::AddSe3PreintEdge(
    int vertex_index1, int vertex_index2,
    const Eigen::Isometry3f &relative_pose, const Eigen::MatrixXf covariance) {
  // INFO("preint edge\t", covariance.diagonal().transpose());
  Matrix6d info = CalculateInformationMatrix(covariance.cast<double>());
  edges_preint_.push_back(EdgePose(vertex_index1, vertex_index2,
                                   relative_pose.cast<double>(), info));
}
void CeresGraphOptimizer::AddSe3LoopEdge(int vertex_index1, int vertex_index2,
                                         const Eigen::Isometry3f &relative_pose,
                                         const Eigen::MatrixXf covariance) {
  Matrix6d info = CalculateInformationMatrix(covariance.cast<double>());
  edges_loop_.push_back(EdgePose(vertex_index1, vertex_index2,
                                 relative_pose.cast<double>(), info));
}
void CeresGraphOptimizer::AddRobustKernel(std::string kernel_type,
                                          double kernel_size) {
  if (kernel_type == "NONE") {
...
Eigen::MatrixXd
CeresGraphOptimizer::CalculateDiagMatrix(Eigen::VectorXd noise) {
...
void CeresGraphOptimizer::AddSe3PriorXYZEdge(int se3_vertex_index,
  ...
  edges_gnss_.push_back(
      EdgePose(se3_vertex_index, se3_vertex_index, pose, info));
}
void CeresGraphOptimizer::AddSe3PriorZEdge(int vertex_index1,
                                           const Eigen::Vector2f height_range,
                                           const Eigen::VectorXf noise) {
  Eigen::Isometry3d pose = Eigen::Isometry3d::Identity();
  pose.translation()(2) = height_range(0);
  Matrix6d info = Matrix6d::Zero();
  info.block<1, 1>(2, 2) = CalculateDiagMatrix(noise.cast<double>());
  edges_abs_height_.push_back(
      EdgePose(vertex_index1, vertex_index1, pose, info));
}
void CeresGraphOptimizer::AddSe3PriordZEdge(int vertex_index1,
                                            int vertex_index2,
  ...
  edges_rel_height_.push_back(
      EdgePose(vertex_index1, vertex_index2, pose, info));
}
void CeresGraphOptimizer::AddSe3PriordRPEdge(int vertex_index1,
                                             int vertex_index2,
  ...
  edges_roll_pitch_.push_back(
      EdgePose(vertex_index1, vertex_index2, pose, info));
}
Eigen::MatrixXd
CeresGraphOptimizer::CalculateInformationMatrix(Eigen::MatrixXd cov) {
...
void CeresGraphOptimizer::DisplayInformation() {
  if (edges_.size() != 0)
    REMIND("odom edge:\ntrace(info) = " +
           std::to_string(EvaluateEdgeInformation(edges_, 1.0)));
  if (edges_preint_.size() != 0)
    REMIND("preint edge:\nweight = " + std::to_string(weight_preint_edge_) +
           "\ntrace(info) = " +
           std::to_string(
               EvaluateEdgeInformation(edges_preint_, weight_preint_edge_)));
  if (edges_loop_.size() != 0)
    REMIND("loop edge:\nweight = " + std::to_string(weight_loop_edge_) +
           "\ntrace(info) = " +
           std::to_string(
               EvaluateEdgeInformation(edges_loop_, weight_loop_edge_)));
  if (edges_gnss_.size() != 0)
    REMIND("gnss edge:\nweight = " + std::to_string(weight_gnss_edge_) +
           "\ntrace(info) = " +
           std::to_string(
               EvaluateEdgeInformation(edges_gnss_, weight_gnss_edge_)));
  std::cout << std::endl;
}
double CeresGraphOptimizer::EvaluateEdgeInformation(const DequeEdgePose &edges,
                                                    double weight) {
  double result = 0.0;
  for (DequeEdgePose::const_iterator it = edges.begin(); it != edges.end();
       ++it) {
