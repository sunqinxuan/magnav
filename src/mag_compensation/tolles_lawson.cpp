...
bool TollesLawson::createMatrixA(const std::vector<double> &Bx,
...
double TollesLawson::createCoeff(
  ...
  if (!createMatrixA(Bx, By, Bz, Bt, terms, Bt_scale)) {
...
bool TollesLawson::bwbp_filter(std::vector<double> &y, const std::vector<double> &x,
    const double pass1, const double pass2, const int pole,
    const int trim)
{
	std::vector<double> bwbpfilter_A=ComputeDenCoeffs(pole,pass1,pass2);
	std::vector<double> bwbpfilter_B=ComputeNumCoeffs(pole,pass1,pass2,bwbpfilter_A);

	if(x.size()<=3*bwbpfilter_A.size() || x.size()<=3*bwbpfilter_B.size())
	{
...
void TollesLawson::linear_regression(std::vector<double> &coeff, std::vector<double> &residual, const std::vector<double> &bb, const std::vector<std::vector<double>> &AA, const double lambda)
{
	const int N=bb.size();
	const int M=AA.size();
	...
	Eigen::MatrixXd ATA=A.transpose()*A+lambda*Eigen::MatrixXd::Identity(M,M);
	Eigen::VectorXd x=ATA.llt().solve(b);
